...
topicsAPI.create = async function (caller, data) {
  ...
  if (!isScheduling) {
    await activitypubApi.create.note(caller, {
...
topicsAPI.migrateThumbs = async (caller, {
  from,
  to
}) => {
  await Promise.all([topicsAPI._checkThumbPrivileges({
    tid: from,
    uid: caller.uid
  ...
  await batch.processArray(tids, async tids => {
    await Promise.all(tids.map(async tid => {
      ...
      if (!topicData.deleted) {
        ...
        activitypubApi.announce.note(caller, {
          tid
        });
        const {
          activity
        } = await activitypub.mocks.activities.create(topicData.mainPid, caller.uid);
