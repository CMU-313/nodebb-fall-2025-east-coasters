...
topicsController.get = async function getTopic(req, res, next) {
  ...
  if (req.params.post_index && !utils.isNumber(req.params.post_index) && req.params.post_index !== 'unread' || !utils.isNumber(tid) && !validator.isUUID(tid)) {
    return next();
  }
  ...
  const [author] = await Promise.all([user.getUserFields(topicData.uid, ['username', 'userslug']), buildBreadcrumbs(topicData), addOldCategory(topicData, userPrivileges), addTags(topicData, req, res, currentPage, postAtIndex), topics.increaseViewCount(req, tid), markAsRead(req, tid), analytics.increment([`pageviews:byCid:${topicData.category.cid}`])]);
